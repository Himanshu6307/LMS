...
function AllCourses() {
    ...
    const filterCategory = () => {
            const courseCopy = courseDetail?.slice();
            if (category.length > 0) {
                const filtered = courseCopy.filter(course => category.includes(course.category));
        ...
        <div className='flex min-h-screen bg-gray-50'>
            ...
            <main className='w-full flex-1 transition-all duration-300 py-[130px] md:pl-[300px] flex items-start justify-center md:justify-between flex-wrap gap-6 px-[10px]'>
                {filteredCourses?.length>0 ? filteredCourses?.map((course, index) => {
                    ...
                    <div className='w-full h-full flex items-center justify-center'><h2 className='text-2xl font-semibold text-gray-600'>No courses available in selected category.</h2></div>}
            </main>
