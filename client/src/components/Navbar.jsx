    ...
    return (
        <div className='w-[100%] h-[65px] px-[20px] py-[10px] flex justify-between items-center fixed top-0 bg-[#00000047] z-10 '>
            ...
            <div className='w-[30%] lg:flex justify-center items-center gap-2 hidden'>
                ...
                {userDetail && <div onClick={() => { setShow((prev) => !prev) }} className='w-[47px] h-[47px] cursor-pointer rounded-full text-red-500 flex justify-center items-center font-bold text-[20px] border-2 bg-black border-white cursor-pointer'>
                    {userDetail?.name.slice(0, 1).toUpperCase()}
                </div>}


                {userDetail?.role === "educator" && <div className='text-white text-[18px]  px-[20px] py-[7px] bg-black border-2 border-white rounded-full cursor-pointer font-light'>Dashboard</div>}
            ...
            <div className={`lg:hidden fixed top-0 left-0  w-[100vw] h-[100vh] bg-[#000000d6] flex items-center justify-center flex-col gap-5 z-10 ${show1 ? "translate-x-[0] transition duration-600" : "translate-x-[-100%] transition duration-600"}`}>
                <MdClose onClick={() => { setShow1((prev) => !prev) }} className='absolute top-6 right-6 text-white w-[30px] h-[30px]' />
                    ...
                    {userDetail && <div  className='w-[47px] h-[47px] cursor-pointer rounded-full text-red-500 flex justify-center items-center font-bold text-[20px] border-2 bg-black border-white cursor-pointer'>
                        {userDetail?.name.slice(0, 1).toUpperCase()}
                    </div>}


                    {userDetail?.role === "educator" && <div className='text-white text-[18px]  px-[35px] py-[9px] bg-black border-2 border-white rounded-xl cursor-pointer font-light'>My Profile</div>}
            ...
            {show && <div className='absolute top-[110%] right-[15%] flex flex-col items-center justify-center gap-2 text-[16px] rounded-md bg-white px-[15px] py-[10px] border-[2px] border-black hover:border-white hover:text-white cursor-pointer hover:bg-black' >
                <span className='bg-black text-white px-[30px] py-[10px] rounded-full hover:bg-gray-600'>My Profile</span>
